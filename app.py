...
# Mixtral is more robust for the free tier than Mistral-7B
MODEL_ID = "mistralai/Mixtral-8x7B-Instruct-v0.1"
csv_path = "freshwater_aquarium_fish_species.csv"

try:
    # Handles potential hidden spaces in secrets
    HF_TOKEN = st.secrets["HF_TOKEN"].strip()
except KeyError:
    st.error("HF_TOKEN not found in Streamlit Secrets.")
    st.stop()

# Increased timeout to 180 seconds to help with "API Busy" or "Cold Starts"
client = InferenceClient(model=MODEL_ID, token=HF_TOKEN, timeout=180)

# --- 3. DATA & RAG RESOURCES ---
@st.cache_resource
def init_resources():
    # Explicitly force CPU for Streamlit Cloud stability
    embedder = SentenceTransformer("all-MiniLM-L6-v2", device="cpu")
    
    # Loading the fish database
    ...
    # Pre-calculate embeddings for semantic search
...
if prompt := st.chat_input("Ask about fish, compatibility, or tank requirements..."):
    ...
    with st.chat_message("assistant"):
        ...
        system_instruction = f"You are AQUARIA, a professional expert. Use this DATA: {context_data}"
        ...
        try:
            ...
            # Displays technical info (like 503 or 429) to help you debug
            st.error(f"
